...
// Extended CV builder helpers
function renderCandidateDashboard(container, tab) {
  if (tab !== 'cv') { container.innerHTML = '<article class="dashboard-card dashboard-highlight"><h3>Twój profil i CV</h3><p>Otwórz kreator CV, aby uzupełnić profil.</p><button class="search-btn" type="button" data-dashboard-tab="cv">Otwórz kreator CV</button></article>'; container.querySelector('[data-dashboard-tab]')?.addEventListener('click', () => renderDashboard('candidate', 'cv')); return }
  const experiences = cvDraft.experiences || [{ company: '', role: '', dates: '', description: '' }]
  const education = cvDraft.educationItems || [{ school: '', degree: '', dates: '', description: '' }]
  const skills = cvDraft.customSkills || candidateSkills.map((name, index) => ({ name, rating: cvDraft.ratings?.[index] || 0 }))
  container.innerHTML = `<article class="dashboard-card dashboard-card-wide cv-builder-pro"><div class="card-heading"><div><span class="eyebrow">Kreator CV Pro</span><h3>CV dopracowane z pomocą AI</h3><p class="helper-text">Dodawaj sekcje, oceniaj umiejętności i przygotuj dokument do druku.</p></div><div class="form-actions"><button type="button" class="btn-secondary" data-print-cv>Drukuj / PDF</button><button type="button" class="search-btn" data-save-cv>Zapisz CV</button></div></div><form id="cvBuilderForm" class="cv-builder-form"><div class="form-grid"><label>Imię i nazwisko<input name="name" required value="${cvDraft.name || ''}"></label><label>Stanowisko docelowe<input name="headline" value="${cvDraft.headline || ''}"></label><label>E-mail<input type="email" name="email" value="${cvDraft.email || ''}"></label><label>Lokalizacja<input name="location" value="${cvDraft.location || ''}"></label></div><label>Podsumowanie zawodowe<textarea name="summary" rows="5">${cvDraft.summary || ''}</textarea><button type="button" class="ai-write" data-ai-target="summary">Napisz z AI</button></label><section class="cv-builder-section dynamic-section"><div class="card-heading"><h4>Doświadczenie</h4><button type="button" class="btn-secondary" data-add="experience">+ Dodaj</button></div><div id="experienceItems">${experiences.map((item, i) => dynamicCvItem('experience', i, item)).join('')}</div></section><section class="cv-builder-section dynamic-section"><div class="card-heading"><h4>Edukacja i certyfikaty</h4><button type="button" class="btn-secondary" data-add="education">+ Dodaj</button></div><div id="educationItems">${education.map((item, i) => dynamicCvItem('education', i, item)).join('')}</div></section><section class="cv-builder-section"><div class="card-heading"><h4>Umiejętności</h4><button type="button" class="btn-secondary" data-add="skill">+ Dodaj umiejętność</button></div><div id="skillItems" class="skills-rating-list">${skills.map((skill, i) => skillEditor(skill, i)).join('')}</div></section><p id="cvBuilderStatus" class="form-status" role="status"></p></form></article>`
  setupCvBuilderPro()
}
function dynamicCvItem(type, index, item) { return `<div class="dynamic-item"><div class="form-grid"><input data-field="${type}" data-index="${index}" data-key="${type === 'experience' ? 'company' : 'school'}" placeholder="${type === 'experience' ? 'Firma' : 'Szkoła / uczelnia'}" value="${item.company || item.school || ''}"><input data-field="${type}" data-index="${index}" data-key="${type === 'experience' ? 'role' : 'degree'}" placeholder="Stanowisko / kierunek" value="${item.role || item.degree || ''}"><input data-field="${type}" data-index="${index}" data-key="dates" placeholder="Daty" value="${item.dates || ''}"></div><textarea data-field="${type}" data-index="${index}" data-key="description" rows="3" placeholder="Opis osiągnięć i obowiązków">${item.description || ''}</textarea><div class="form-actions"><button type="button" class="ai-write" data-ai-item="${type}" data-index="${index}">Napisz z AI</button><button type="button" class="btn-danger" data-remove="${type}" data-index="${index}">Usuń</button></div></div>` }
function skillEditor(skill, index) { return `<div class="skill-rating dynamic-item"><input data-skill-name="${index}" placeholder="Nazwa umiejętności" value="${skill.name || ''}"><div class="rating-stars">${[1,2,3,4,5].map((n) => `<button type="button" class="skill-star ${(skill.rating || 0) >= n ? 'is-selected' : ''}" data-custom-skill="${index}" data-rating="${n}">★</button>`).join('')}</div><button type="button" class="btn-danger" data-remove="skill" data-index="${index}">Usuń</button></div>` }
async function generateCvText(type, context) { const response = await fetch('/api/cv-description', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type, context }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error); return data.text }
function setupCvBuilderPro() { const form = document.getElementById('cvBuilderForm'); const collect = () => { Object.assign(cvDraft, Object.fromEntries(new FormData(form).entries())); cvDraft.experiences = [...document.querySelectorAll('[data-field="experience"]')].reduce((all, el) => { const i = Number(el.dataset.index); all[i] = all[i] || {}; all[i][el.dataset.key] = el.value; return all }, []); cvDraft.educationItems = [...document.querySelectorAll('[data-field="education"]')].reduce((all, el) => { const i = Number(el.dataset.index); all[i] = all[i] || {}; all[i][el.dataset.key] = el.value; return all }, []); cvDraft.customSkills = [...document.querySelectorAll('[data-skill-name]')].map((el, i) => ({ name: el.value, rating: cvDraft.customSkills?.[i]?.rating || 0 })); localStorage.setItem('jobnexus-cv', JSON.stringify(cvDraft)) }; document.querySelectorAll('[data-add]').forEach((b) => b.addEventListener('click', () => { collect(); const key = b.dataset.add === 'skill' ? 'customSkills' : b.dataset.add === 'experience' ? 'experiences' : 'educationItems'; cvDraft[key].push(b.dataset.add === 'skill' ? { name: '', rating: 0 } : {}); renderCandidateDashboard(document.getElementById('accountCards'), 'cv') })); document.querySelectorAll('[data-remove]').forEach((b) => b.addEventListener('click', () => { collect(); const key = b.dataset.remove === 'skill' ? 'customSkills' : b.dataset.remove === 'experience' ? 'experiences' : 'educationItems'; cvDraft[key].splice(Number(b.dataset.index), 1); renderCandidateDashboard(document.getElementById('accountCards'), 'cv') })); document.querySelectorAll('.skill-star').forEach((b) => b.addEventListener('click', () => { collect(); cvDraft.customSkills[Number(b.dataset.customSkill)].rating = Number(b.dataset.rating); renderCandidateDashboard(document.getElementById('accountCards'), 'cv') })); document.querySelectorAll('[data-ai-target]').forEach((b) => b.addEventListener('click', async () => { const field = form.elements[b.dataset.aiTarget]; field.value = await generateCvText(b.dataset.aiTarget, `${cvDraft.headline || ''} ${field.value}`) })); document.querySelectorAll('[data-ai-item]').forEach((b) => b.addEventListener('click', async () => { collect(); const list = b.dataset.aiItem === 'experience' ? cvDraft.experiences : cvDraft.educationItems; const item = list[Number(b.dataset.index)] || {}; const target = document.querySelector(`[data-field="${b.dataset.aiItem}"][data-index="${b.dataset.index}"][data-key="description"]`); target.value = await generateCvText(b.dataset.aiItem, JSON.stringify(item)) })); document.querySelector('[data-save-cv]').addEventListener('click', () => { collect(); document.getElementById('cvBuilderStatus').textContent = 'CV zapisane.' }); document.querySelector('[data-print-cv]').addEventListener('click', () => { collect(); window.print() }) }
function setupCvBuilder() { setupCvBuilderPro() }
